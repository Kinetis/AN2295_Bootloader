...
FMSTR_BOOL FMSTR_CheckTsaSpace(FMSTR_ADDR dwAddr, FMSTR_SIZE8 nSize, FMSTR_BOOL bWriteAccess)
{
    ...
    for(nTableIndex=0U; (pte=FMSTR_TsaGetTable(nTableIndex, &cnt)) != NULL; nTableIndex++)
    {
        ...
        for(i=0U; i<cnt; i++)
        {
            #pragma diag_suppress = Pe767 /* Disable Warning [Pe767]: conversion from pointer to smaller integer */          
            ...
            #pragma diag_warning = Pe767 /* ReEnable Warning [Pe767]: conversion from pointer to smaller integer */                    
